...
1. **相锻造** — 锻造台不吃固定配方。材料不用摆格子:**手持右键投进台子**(潜行投整组,扔到台面上也会被吸收),台面上方漂浮材料虚影实时可见;**空手右键直接拿产物**,潜行+空手右键取回全部材料。原版物品也是零件,产物属性完全由材料的品阶、相性、功能算子决定(最多 25 件,超过 10 件的零件强度分按半权重计入,防线性膨胀)。GUI 保留 AI 推荐/蓝图面板(去网格,材料区为文字列表)。
...
生存流程:拾相材料 → 合成锻造台 → 锻造增幅器(法杖/魔法书) → 合成炼金台 → 摆料或问 AI 炼卷轴 → 右键学习 → 按住 G 轮盘施法 → 集裂隙精髓 → 右键裂隙岩进万象森罗 → 打守望者 → 传奇材料。成就树(进度 → 千相)全程引导。

测试:`/qianxiang kit` 一键领全套;`/qianxiang dim` 快速往返维度。测试链:kit → 锻造增幅器 → 炼金台 AI 卷轴 → 右键学习 → G 轮盘施法。
