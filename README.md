...
1. **相锻造** — 锻造台不吃固定配方。材料不用摆格子:**手持右键投进台子**(潜行投整组,扔到台面上也会被吸收),台面上方漂浮材料虚影实时可见,潜行+空手右键取回全部材料。产物就绪后点「开始创作」(产物槽/空手右键同效)触发**合成仪式**:GUI 关闭,材料飞入台中化为粒子——炼金台在空中画出双层反向魔法阵,锻造台则是锻打火花——仪式完成后产物留在台面,空手右键拾取。原版物品也是零件,产物属性完全由材料的品阶、相性、功能算子决定(最多 25 件,超过 10 件的零件强度分按半权重计入,防线性膨胀)。GUI 保留 AI 推荐/蓝图面板(去网格,材料区为文字列表)。
...
3. **自由法术** — 9 元素 × 5 形式 × 5 效果 × 5 修饰的组合法术引擎。九元素各有专属机制(不全是伤害:火点燃、冰霜冻、雷链、自然骨粉/毒、暗影致盲隐身、神圣克亡灵群疗、鲜血吸血换蓝、末影传送漂浮、奥术印记夜视),debuff/utility 九格效果互不重复;单发伤害对标同档武器(power×3.0,模板 power=2×(档位+1))。法杖/魔法书是**增幅器**(法术伤害 +%、法力上限 +X,主手+副手同时生效、效果叠加),不再承载法术;法术本体来自**炼金台**:摆材料或问 AI 炼出魔法卷轴(~26 种模板,含双算子组合技,AI 可生成专属法术) → 右键学习进已学列表 → 按住 G 弹出轮盘,指向扇区松开即施放(点按 G 快速施放上次法术)。
...
生存流程:拾相材料 → 合成锻造台 → 锻增幅器(法杖/魔法书,开始创作走一段仪式) → 合成炼金台 → 摆料或问 AI 炼卷轴(同样仪式出炉) → 右键学习 → 按住 G 轮盘施法 → 集裂隙精髓 → 右键裂隙岩进万象森罗 → 打守望者 → 传奇材料。成就树(进度 → 千相)全程引导。

测试:`/qianxiang kit` 一键领全套;`/qianxiang dim` 快速往返维度。测试链:kit → 投料锻造增幅器(开始创作→仪式→台面拾取) → 炼金台 AI 卷轴(仪式) → 右键学习 → G 轮盘施法。
