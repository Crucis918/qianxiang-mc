...
## WQ-95 [x] 完成(acd66fb) 【大·荣耀风战斗】PVP 镜头 + 连招 + 千机伞形态转换 + 华丽特效
...
## WQ-96 [x] 完成(d1c8553) 【大·主职业】自定义职业内核 + 存档隔离事故修复

**范围**:① 主职业——玩家自选 2 元素+1 武器形态组成职业内核(全自定义,另给 8 预设
模板一键选):内核元素法术 ×1.15 伤/×0.9 蓝,非内核 ×0.45 伤/×1.25 蓝(可用但弱);
内核形态武器 ×1.05,非内核 ×0.6;未设内核全 ×1.0(旧存档兼容)。相师处技能树第 4
页签设定,首次免费,转职 10 绿宝石;轮盘非内核法术压暗 50% 显示职业边界。近战倍率
只拦 PLAYER_ATTACK(防误乘法术 AoE,测试抓出)。② 事故修复:游玩检测复位清了用户
存档的已学法术/熟练度(用户实机投诉)——quickPlay 默认改专用存档「游玩检测」并立
铁律;新增 /qianxiang learn <id|all> 与 /qianxiang prof addxp 恢复命令。

**验收**:160/160 GameTest(新增 5);Xvfb 下游检测在专用存档 27/27 全 PASS,
用户存档零接触。

**遗留观察**:① learn 命令只补预置法术,卷轴学的 forged_* 需重新炼金(数据在卷轴
组件);② 非内核削弱只作用伤害/蓝耗/近战,治疗/buff/utility 未削(治疗职业边界
待观察);③ 职业与连招/增幅器全乘区叠乘,数值上限需实机调平。

---

## WQ-97 [x] 完成(a464b2b) 【大·职业】全职高手·荣耀 六系 24 职业

**范围**:主职业从 8 模板扩到荣耀六系 24 职业,每职业=内核+招牌被动:剑士(剑客/
魔剑士/狂战士/阵鬼)、枪手(神枪手·弹体线/弹药专家/机械师·铁傀儡伙伴/枪炮师)、
格斗(拳法家·蓄力加速/柔道家/气功师/流氓)、法师(元素法师/战斗法师·连招上限12/
召唤师·狼伙伴/魔道学者)、暗夜(刺客·背刺/盗贼·击杀隐身/死灵术士/忍者)、圣职
(牧师·治疗+30%/圣骑士·减伤/驱魔师/复仇者)。同内核职业(剑客vs战斗法师、流氓vs
盗贼)靠 classTemplateId 区分被动;自定义内核保留;UI 按系列分组;伙伴 60s 重生。
弹速挂点=castProjectile 速度参数乘区(神枪手 ×1.5)。

**验收**:167/167 GameTest(新增 7);Xvfb 回归 27/27 全 PASS。

**遗留观察**:① 枪手系走弹体法术线(无枪械物品,设计理念映射);② 死灵术士无召唤
(原版骷髅敌我 AI 难共处,给亡灵主题被动替代);③ 伙伴不进万象森罗跨维度重生逻辑
未特判;④ 24 职业平衡未实机调(全部常量集中 ClassCoreHelper 顶部)。

---

## WQ-98 [x] 完成(9592a1a) 【大·职业技能】24 职业 × 2 主动技能 + 自创魔法贴合职业
...
## WQ-99 [x] 完成(b2ce937) 【中·主题】职业体系原创换皮(脱荣耀)

**范围**:用户澄清荣耀只是举例——六系 24 职业与 48 技能整体换原创名:刃系(相剑士/
霜刃/裂刃狂/影镰)、弹系(贯星者/焰雨/械师/破城炮)、拳系(疾拳/山崩/御气/毒手)、
术系(元素相师/斗术师/兽契者/玄机)、影系(影袭/夜行/亡语/瞬身)、辉系(辉医/磐辉/
破邪/血誓);技能名全原创(连星斩/惊雷突/挑月/裂空连刺/狼契增援/背袭/辉光普照…)。
**id 全保留只换显示名**——存档 classTemplateId/冷却表/payload/测试零迁移,注释留档。

**验收**:175/175 GameTest;lang 995/995 零差集。

---

## WQ-100 [x] 完成(9f6b4a6) 【大·体验+经济】AI 动作编排/自动备料/口语理解 + 吸收白名单/右键优先级 + 属性审计 + 黄金经济

**范围**(实机投诉七件事):
① **AI 动作编排**——动作编辑器加「AI 编排」:自然语言描述招式(「三段快斩接一次重劈」)
→ MovesetComposer(AI prompt 含语义指引:连快=低 power 高 speed/重劈=高 power 低 speed/
突=DASH/扫=连击+2 few-shot;离线关键词兜底)→ 序列回填可微调。
② **自动备料**——方案卡区「自动备料」按钮一键从背包+存储拉齐材料,报来源与缺料。
③ **提示词理解**——prompt 加口语理解段(summary 先复述意图,不确定给 questions),
「示例」按钮轮换示例,兜底关键词口语化(猛/狠/帅/快)。
④ **吸收白名单**——台子只吸显式材料(PhaseData/功能 tag/相材概念),猪肉种子不再被吞;
不用推导兜底口径(食物→HEAL 那种),注释留档。
⑤ **右键不再误触发**——空手右键永远开 GUI,仪式只从「开始创作」按钮/产物槽触发;
潜行取回优先级不变。
⑥ **属性提取审计**——箭=EDGE、金粒=仅 MANA(硬币不当骨架)、金系装备+MANA、药水
=MANA、有害食物=POISON 不再误给 HEAL;22 类物品矩阵测试锁定。
⑦ **黄金经济**——金粒=世界硬币:试炼保底/敌怪 5%/遗迹箱子/守望者都掉金;洗点 30 金粒、
转职 45 金粒(绿宝石退役)、深渊商人金粒计价+卖料换金(档位 1/3/6/12,日限 8 次)。

**验收**:201/201 GameTest(新增 23);Xvfb 回归 27/27 全 PASS;lang 1012/1012 零差集。

**遗留观察**:① parseMovesetJson 的动画真实性校验只查 namespace+prefix 不查库登记
(现状语义,编造的 epicfight:biped/combat/xxx 会放行);② PlaytestHandler:881/917 注释
还是旧吸收行为描述(下次装置维护时改);③ 弹体系职业无真枪械(设计理念映射留档)。
